...
class UserBlueprint(Blueprint):
    ...
    def _add_routes(self):
        self.add_url_rule(
            "/user", "user", login_required(self.get_user), methods=["GET"]
        )
    ...
    def get_user(self):
        ...
        return render_template(
            "user.html", user_info=user_info, linked_characters=linked_characters
        )
    ...
    def _fetch_and_cache_character_info(self, user):
        ...
        data["alliance_name"] = (
            self._alliance_id_to_name(alliance_id) if alliance_id else None
        )
        data["corporation_name"] = (
            self._corporation_id_to_name(corporation_id) if corporation_id else None
        )
        ...
        data["wallet_balance"] = (
            wallet_data if wallet_status == 200 and wallet_data is not None else 0.0
        )
