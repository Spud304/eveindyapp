...
from src.models.models import db, InvTypes, CachedLocations, CachedBlueprint, IndustryActivityProducts, IndustryBlueprints, MapSolarSystems
...
from src.industry_utils import load_rig_data, load_sde_manufacturing_data, load_type_group_category_maps, pick_best_station, classify_product_for_rig
...
class IndustryBlueprint(Blueprint):
    ...
    def get_blueprint_materials(self, type_id: int):
        ...
        materials_by_bp, products_by_product, _bp_to_product = load_sde_manufacturing_data()
    ...
    def get_calculator(self):
        ...
        # Pre-load SDE data for DFS
        materials_by_bp, products_by_product, bp_to_product = load_sde_manufacturing_data()
        ...
        for bp_id in tree_bp_ids:
            ...
            if prod_id:
                rig_cat = classify_product_for_rig(prod_id, type_to_group, group_to_category)
            else:
                rig_cat = None
            bp_rig_categories[bp_id] = rig_cat

            if rig_cat and stations:
                best_station, best_me = pick_best_station(stations, rig_cat, system_securities, rig_data)
