...
def upgrade():
    with op.batch_alter_table('user', schema=None) as batch_op:
        batch_op.add_column(sa.Column('main_character_id', sa.BigInteger(), nullable=True))

    # Backfill: each existing user becomes their own main
    op.execute('UPDATE user SET main_character_id = character_id WHERE main_character_id IS NULL')
